...
@onready var correct_answer_buttons: Array[Button] = [
	$animatedBackground/correct1,
	$animatedBackground/correct2,
	$animatedBackground/correct3,
	$animatedBackground/correct4,
]
@onready var favButton = $animatedBackground/fav
@onready var editButton = $animatedBackground/edit
...
func _ready() -> void:
	...
	loader.preLoad()
	flashcards = loader.get_custom_flashcards()
...
func _on_next_pressed() -> void:
	current_index += 1
	if current_index > flashcards.size() - 1:
		current_index = 0
	show_flashcard(current_index)
...
func _on_correct_1_pressed() -> void:
	correctAnswerIndex = 0
	change_correct_answer(correctAnswerIndex)

func _on_correct_2_pressed() -> void:
	correctAnswerIndex = 1
	change_correct_answer(correctAnswerIndex)

func _on_correct_3_pressed() -> void:
	correctAnswerIndex = 2
	change_correct_answer(correctAnswerIndex)

func _on_correct_4_pressed() -> void:
	correctAnswerIndex = 3
	change_correct_answer(correctAnswerIndex)

func change_correct_answer(index):
	flashcards[current_index]["answer"] = flashcards[current_index]["choices"][index]
	for i in range(answer_buttons.size()):
		var btn: Button = answer_buttons[i]
		if  flashcards[current_index]["choices"][i] == flashcards[current_index]["answer"]:
			btn.add_theme_stylebox_override("normal", correct_style)
			btn.add_theme_stylebox_override("hover", correct_style)
...
func add_new_card():
	...
	current_index = flashcards.size() - 1
	flashcards[current_index]["isCustom"] = true
	save_settings(flashcards[current_index]["isCustom"])
	print(flashcards[current_index]["question"])
	show_flashcard(current_index)
...
func _on_delete_pressed() -> void:
	if flashcards.size() == 0:
		return
	flashcards.remove_at(current_index)
	for i in range(current_index, flashcards.size()):
		flashcards[i]["id"] -= 1
